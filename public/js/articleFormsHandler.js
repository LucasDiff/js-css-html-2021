...
export default class articleFormsHandler {
    ...
    assignFormAndArticle(formElementId, cssClass2hideElement,articleId,offset,totalCount){
        ...
        if(articleId>=0){ //article edit (update)
            artForm.onsubmit= (event)=>this.processArtEditFrmData(event, false);
            this.articleId = articleId;
            this.offset = offset;
            this.totalCount = totalCount;
        }else{
            artForm.onsubmit= (event)=>this.processArtEditFrmData(event, true);
            this.articleId = articleId;
            this.offset = offset;
            this.totalCount = totalCount;
            if (auth2.isSignedIn.get()) {
                this.formElements.namedItem('author').value = auth2.currentUser.get().getBasicProfile().getName();
            } else {
     ...
     */
    processArtEditFrmData(event, newArticle) {
        ...
        if (!newArticle) {
            ...
            fetch(`${this.serverUrl}/article`, postReqSettings)  //now we need the second parameter, an object wih settings of the request.
                ...
                .then(responseJSON => { //here we process the returned response data in JSON ...
                    window.alert("New article saved on server");
                })
                .catch(error => { ////here we process all the failed promises
